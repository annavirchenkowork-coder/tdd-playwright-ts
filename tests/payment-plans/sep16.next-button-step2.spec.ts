...
test.describe(
  ...
  () => {
    ...
    // =========================================================
    test(
      "AC1 - Next button activates after selecting a plan @sep16-1",
      async ({ page }) => {
    ...
    // =========================================================
    test(
      "AC2 - Stepper colors update when proceeding to Step 3 @sep16-2",
      async ({ page }) => {
    ...
    // =========================================================
    test(
      "AC3 - Price summary displays correctly for both plans @sep16-3",
      async ({ page }) => {
        const { paymentPlan } = initPages(page);
        ...
        await paymentPlan.selectPaymentPlan("upfront");
        await expect(
          paymentPlan.basePriceAmountUnderUpfront
        ).toBeVisible();
        await expect(
          paymentPlan.upfrontDiscountAmountUnderUpfront
        ).toBeVisible();
        await expect(
          paymentPlan.subtotalAmountUnderUpfront
        ).toBeVisible();
    ...
    // =========================================================
    test(
      "AC4 - Back button works & returns to Step 1 @sep16-4",
      async ({ page }) => {
        const { startApp, paymentPlan } = initPages(page);
